...
namespace ar_pose
{
  ...
  distCoeffs(1, 4, CV_64F)
  {
    ...
    if (!use_camera_info)
    {
      ...
      cam_param_.dist_factor[0] = px;       // x0 = cX from openCV calibration
      cam_param_.dist_factor[1] = py;       // y0 = cY from openCV calibration
      ...
      distCoeffs.at<double>(0) = cam_d0;
      distCoeffs.at<double>(1) = cam_d1;
      distCoeffs.at<double>(2) = cam_d2;
      distCoeffs.at<double>(3) = cam_d3;
    ...
    // **** advertsie
    tagPosePub_ = n_.advertise<geometry_msgs::PoseStamped>("tag_pose", 0);
  ...
  void ARSinglePublisher::getTransformationCallback (const sensor_msgs::ImageConstPtr & image_msg)
  {
    ...
    cv::Mat img2;
    cv::undistort(capture_->image, img2, cameraMatrix, distCoeffs);
    ...
    if (k != -1)
    {
      ...
      if(publishTf_)
      {
        if(reverse_transform_)
        {
          ...
          tf::StampedTransform markerToCam (tInv, image_msg->header.stamp, markerFrame_.c_str(), "cam_pos");
          broadcaster_.sendTransform(markerToCam);
        }
        else
        {
          tf::StampedTransform camToMarker (t, image_msg->header.stamp, "cam_pos", markerFrame_.c_str());
